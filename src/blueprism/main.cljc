(ns blueprism.main
  "Kotodama WASM entrypoint for the Blueprism clean-room actor (L4) — Clojure port.
  ...
  facts (`blueprism.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "blueprism")
(def tier "L4")
...
(def entity-specs
  [{:entity "Bot"           :plural "bots"          :id-prefix "bluepris_bot"
    :fields [:name :machine :status]                :required [:name :machine]
    :coerce {}                       :refs {}}
   {:entity "Process"       :plural "processes"     :id-prefix "bluepris_pro"
    :fields [:name :version :published]             :required [:name :version]
    :coerce {:published :bool}       :refs {}}
   {:entity "Job"           :plural "jobs"          :id-prefix "bluepris_job"
    :fields [:processId :botId :status :startedAt]  :required [:status :startedAt]
    :coerce {}                       :refs {:processId "Process" :botId "Bot"}}
   {:entity "Queue"         :plural "queues"        :id-prefix "bluepris_que"
    :fields [:name :pending :priority]              :required [:name :pending]
    :coerce {:pending :int}          :refs {}}
   {:entity "Asset"         :plural "assets"        :id-prefix "bluepris_ass"
    :fields [:name :type :valueRef]                 :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "ProcessMining" :plural "processminings" :id-prefix "bluepris_pro"
    :fields [:name :caseCount :variants]            :required [:name :caseCount]
    :coerce {:caseCount :int :variants :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"blueprism.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "blueprism-compat" :tier tier :entities entities} 200])
...
(defn start! [] :blueprism-compat/ready)
